            ...
            if (!isGameOver)
            {
                ...
                if (playerRankings.Length > 0)
                {
                    ...
                    if (!isDraw)
                    {
                        ...
                        // Lấy phần tử đầu tiên trong mảng ps
                        PlayerInstance topPlayer = ps.Length > 0 ? ps[0] : null;

                        if (topPlayer != null)
                        {
                            Debug.Log("Người chơi đứng đầu là: " + topPlayer.playerName);
                        }
                        else
                        {
                            Debug.Log("Danh sách người chơi trống.");
                        }

                        if (photonView.IsMine) {
                            // Gán playerName từ PhotonNetwork.NickName
                            playerName = photonView.Owner.NickName;
                            Debug.Log("playerName: " + playerName);

                            // Kiểm tra nếu đây là đối tượng của người chơi hiện tại và tên của người chơi hiện tại trùng với topPlayer
                            if (playerName == topPlayer.playerName)
                            {
